...
async function bootstrap() {
  ...
  app.useGlobalPipes(//what is a global pipe? A global pipe is a way to apply a transformation or validation logic to all incoming requests in the application. By using app.useGlobalPipes(), we can set up a pipe that will run for every request that hits our controllers, allowing us to automatically validate and transform the incoming data according to the rules defined in the pipe. In this case, we are using the built-in ValidationPipe from NestJS, which provides powerful validation capabilities based on class-validator decorators defined in our DTOs (Data Transfer Objects). This means that any request that contains data matching our DTOs will be automatically validated against the rules we have set, and if the validation fails, an appropriate error response will be sent back to the client without needing to write manual validation logic in each controller method.
    new ValidationPipe({//for dto validation, it will automatically validate the incoming request data against the rules defined in our DTO classes using class-validator decorators. If the validation fails, it will return a 400 Bad Request response with details about the validation errors. The options we have set here include:
      whitelist: true,
