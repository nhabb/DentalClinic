...
export class PatientRecordsController {
  ...
  create(@Body() dto: CreatePatientRecordDto) {//it is sent in the request body as JSON, used for POST/PUT requests. Here we expect a JSON object that matches the CreatePatientRecordDto structure, which includes properties like patient_id, record_type, description, and optionally file_url to create a new patient record.
