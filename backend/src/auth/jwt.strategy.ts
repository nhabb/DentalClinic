...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  constructor(private readonly prisma: PrismaService) {
    //why is took prisma as a dependency? Because we need to look up the user in the database based on the user ID (sub) from the JWT token payload to validate that the user exists and is active. By injecting the PrismaService, we can easily access our database and perform this lookup in the validate() method of our JwtStrategy.
    super({
      //super() calls the constructor of the parent class (PassportStrategy) and passes it the configuration object for the JWT strategy. This configuration includes:
